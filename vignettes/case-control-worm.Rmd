...
This explainer vignette walks you through each of these two steps. 

Step 1 requires two sources of data: survey data with labels for "cases" and "controls" (as well as any relevant covariates) and a shapefile for the geographical region under investigation; Step 2 requires these two data sources prepared in list format to be passed to the estimating equation. 
...
|----------|:-------------:|
|shape | shapefile data
|survey | sase-control data including common geographic ID
|shape_large.area_id_name | large area name identifiers in the shapefile
|shape_large.area_id_num | large area numeric identifiers in the shapefile
|shape_small.area_id_name | small area name identifiers in the shapefile
|shape_small.area_id_num | small area numeric identifiers in the shapefile
|survey_small.area_id_name |small area name identifiers in the survey
|survey_small.area_id_num | small area numeric identifiers in the survey
|drop.incomplete.records | should the function return complete data?
|colnames_X | a character vector of covariates definining the design matrix X. Note these should be numeric
|interactions_list | a list object where each element is a string of the form "a*b" where a and be are the names of two variables in colnames_X
|scale_X | takes values "1sd" or "2sd"
|colname_y | numeric variable name for the outcome variable
|contamination | should this offset account for contamination?
|pi | a scalar defining the prevalence of the outcome in the population of interest
|large_area_shape | should the function return a large-area shapefile?
